  ...
  template: `<article>
    <div class="visual">
      <a [routerLink]="['/stays', item().slug]" [attr.aria-label]="'View ' + item().name">
        <sp-image [src]="item().cover_image" [alt]="imageAlt()" />
      </a>
      <span class="type">{{ item().stay_type }}</span>
      <button
        type="button"
        class="fav"
        ...
        [attr.aria-label]="favourited() ? 'Remove from saved stays' : 'Save stay'"
      >
        <sp-icon name="heart" />
      </button>
    </div>
    <div class="body">
      <p class="price">
        @if (price()) {
          From {{ price() }} <small>/ night</small>
        } @else {
          Price on request
        }
      </p>
      <h3><a [routerLink]="['/stays', item().slug]">{{ item().name }}</a></h3>
      <p class="location"><sp-icon name="pin" />{{ location() }}</p>
      @if (item().available_room_type_count) {
        <p class="rooms">
...
export class StayCardComponent implements OnInit {
  ...
  price() {
    return formatMoney(this.item().minimum_nightly_price);
  }
  ...
  imageAlt() {
    return [this.item().name, this.location()].filter(Boolean).join(' in ');
  }

  toggleFavourite() {
    if (!this.auth.isAuthenticated()) {
      void this.router.navigate(['/login'], { queryParams: { returnUrl: `/stays/${this.item().slug}` } });
      return;
    }
