  ...
  template: `<aside aria-live="polite" aria-label="Notifications">
    @for (toast of service.items(); track toast.id) {
      <article
        ...
        [class.loading]="toast.kind === 'loading'"
        [class.action]="toast.kind === 'action'"
        ...
        <div>
          ...
          @if (toast.action) {
            <button
              class="toast-action"
              type="button"
              (click)="toast.action.run(); service.dismiss(toast.id)"
            >
              {{ toast.action.label }}
            </button>
          }
  ...
  styles: [
    `
      ...
      .toast {
        ...
        background: #fff;
        box-shadow: 0 16px 38px rgba(21, 43, 42, 0.14);
        pointer-events: auto;
      ...
      .info,
      .loading {
        border-left-color: var(--verification);
      }
      .info > i,
      .loading > i {
...
export class ToastRegionComponent {
  ...
  icon(toast: Toast) {
    return toast.kind === 'success' || toast.kind === 'action'
