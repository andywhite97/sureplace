...
@Component({
  ...
  template: `@if(visible()){
    <nav aria-label="Mobile bottom navigation">
      <a routerLink="/" routerLinkActive="active" [routerLinkActiveOptions]="{exact:true}"><i class="fa-solid fa-compass" aria-hidden="true"></i><small>Explore</small></a>
  ...
  styles: [
    `nav{display:none}@media(max-width:700px){nav{position:fixed;z-index:30;bottom:0;left:0;right:0;display:flex;justify-content:space-around;background:#fff;border-top:1px solid var(--line);padding:.45rem env(safe-area-inset-right) calc(.45rem + env(safe-area-inset-bottom)) env(safe-area-inset-left);box-shadow:0 -8px 24px rgba(21,43,42,.07)}a{position:relative;display:grid;min-width:4rem;min-height:48px;justify-items:center;align-content:center;gap:.18rem;color:var(--slate);text-decoration:none;font-size:1rem}a>i,.profile-initials,a>img{width:34px;height:30px;display:grid;place-items:center;border-radius:.7rem;font-style:normal}a>img{width:30px;height:30px;border-radius:50%;object-fit:cover}.profile-initials{width:30px;height:30px;border-radius:50%;background:var(--mist);color:var(--teal);font-size:.68rem;font-weight:850}.active{color:var(--teal);font-weight:800}.active>i{background:color-mix(in srgb,var(--teal) 12%,white)}small{font-size:.68rem}b{position:absolute;top:.05rem;right:.85rem;min-width:1.2rem;text-align:center;border:2px solid #fff;border-radius:999px;background:var(--teal);color:#fff;font-size:.62rem;padding:.06rem .25rem}}`,
  ],
})
